...
function woocommerce_gateway_razorpay_init() {
	...
	class WC_Gateway_Razorpay extends WC_Payment_Gateway {
		...
		public function __construct(){
			...
			$this->enabled				= $this->settings['enabled']; //Enabled Status for validation.
			...
			$this->logo 			= $this->settings['logo']; // URL of Merchant Logo.
			...
			$this->theme_color 		= $this->settings['theme_color']; // Brand color to alter the appearance of checkout form.
		
            $this->debug 			= $this->settings['debug']; //Display settings error on front end.
		...
		function init_form_fields(){
			...
			$this->form_fields = array(
				...
				// Description as displayed on Frontend
  				...
  				// Merchant Logo on Checkout Modal
      			'logo' => array(
					'title' 		=> __('Logo URL:', 'razorpay'),
					'type' 			=> 'text',
					'description' 	=> __('<strong><a class="logo_upload">Select Logo</a></strong><br/>The URL for the logo file, displayed on the Checkout Page.<br/>Must start with "<strong>http://</strong>" -or- "<strong>https://</strong>". [max: 56x56 px | file: png/jpg/gif/ico]', 'razorpay'),
					'class'			=> 'logo',
					'desc_tip' 		=> false
				),
				// LIVE Key-ID
				...
				// TEST Key-ID
  				...
  				// Merchant Logo on Checkout Modal
      			'theme_color' => array(
					'title' 		=> __('Theme Color:', 'razorpay'),
					'type' 			=> 'text',
					'description' 	=> __('Brand color to alter the appearance of checkout form', 'razorpay'),
					'default' 		=> '#8058a5',
					'css'			=> 'width:75px',
					'class'			=> 'iris_color_picker',
					'desc_tip' 		=> true
				),
  				// DEBUG
      			'debug' => array(
					'title' 		=> __('Debug', 'kdc'),
					'type' 			=> 'checkbox',
					'label' 		=> __('Display Error on Front end.', 'razorpay'),
					'default' 		=> 'no',
					'description' 	=> __('Tick to display setting error! NOTE: Erros will be visable on the front end.'),
					'desc_tip' 		=> true
                ),
			);
		...
		function validate_fields(){
	 		...
	 		$razorpay_validate_error = 0; //Declear No-Error to start with.

			//Check if_enabled
			if ( $this->enabled == 'yes' && $this->debug == 'yes' ) {			
				if ( $this->logo != '' ) {
					$check_url = razorpay_check_file($this->logo);
					if ( $check_url[0] != 'ok' ) {
						foreach ( $check_url as $error ){
							wc_add_notice( $error, 'error' );				
						}
						$razorpay_validate_error=1;
					}//END-check_url
				}//END-check_logo
				// Check test_mode to determine the Key-Id and Key-Secret required & Valid
				if ( $this->test_mode =='test' ){
					if ( $this->key_id_test == '' ) { //Check if key_id_test is filled
						wc_add_notice( __('URL Mode is set to "Test", so "[TEST] Key Id" is required', 'razorpay'), 'error' );				
						$razorpay_validate_error=1;
					}
					if ( $this->key_secret_test == '' ) { //Check if key_secert_test is filled
						wc_add_notice( __('URL Mode is set to "Test", so "[TEST] Key Secret" is required', 'razorpay'), 'error' );				
	 					$razorpay_validate_error=1;
					}
					if ( $this->key_id_test != '' && $this->key_secret_test != '' && strtolower(substr($this->key_id_test, 0, 9)) != 'rzp_test_' ){
						wc_add_notice( __('"[TEST]Key Id" must start with "rzp_test_". You can get it from ', 'razorpay').'<a href="https://dashboard.razorpay.com/#/app/keys" target="_blank">'. __('Razorpay Dashboard > Api Keys', 'razorpay').'</a>'.__(' in "Test Mode"', 'razorpay'), 'error' );
	 					$razorpay_validate_error=1;
					}
				} else {
					if ( $this->key_id == '' ) { //Check if key_id is filled
						wc_add_notice( __('URL Mode is set to "Live", so "Key Id" is required', 'razorpay'), 'error' );				
	 					$razorpay_validate_error=1;
					}
					if ( $this->key_secret == '' ) { //Check if key_secert is filled
						wc_add_notice( __('URL Mode is set to "Live", so "Key Secret" is required', 'razorpay'), 'error' );				
	 					$razorpay_validate_error=1;
					}
					if ( $this->key_id != '' && $this->key_secret != '' && strtolower(substr($this->key_id, 0, 9)) != 'rzp_live_' ){
						wc_add_notice( __('"Key Id" must start with "rzp_live_". You can get it from ', 'razorpay').'<a href="https://dashboard.razorpay.com/#/app/keys" target="_blank">'. __('Razorpay Dashboard > Api Keys', 'razorpay').'</a>'.__(' in "Live Mode"', 'razorpay'), 'error' );
	 					$razorpay_validate_error=1;
					}
				}//END-{else}-check_test_mode
			}//END-check_enabled
			
			// Check if any validation ERROR occured and return;
			if ( $razorpay_validate_error == 1 ) {
				return false;
			} else {
				return true;
			}//END-check_error
		...
		function generate_razorpay_form($order_id){
			...
			// Theme Color
			if ( isset( $this->theme_color ) && $this->theme_color != '' ) {
				$razorpay_args['theme']	= array( 'color' => $this->theme_color );
			}
...
**/
add_filter( 'plugin_action_links', 'razorpay_add_action_plugin', 10, 5 );
function razorpay_add_action_plugin( $actions, $plugin_file ) {
	static $plugin;
	...
	if ($plugin == $plugin_file) {
			...
			$settings = array('settings' => '<a href="admin.php?page=wc-settings&tab=checkout&section=wc_gateway_razorpay">' . __('Settings') . '</a>');
...
* JS Dependency
**/
add_action( 'admin_enqueue_scripts', 'razorpay_js_dependency' );
function razorpay_js_dependency( $hook_suffix ) {
    // COLOR-PICKER
	// first check that $hook_suffix is appropriate for your admin page
    wp_enqueue_style( 'wp-color-picker' );
    wp_enqueue_script( 'razorpay-color-picker', plugins_url('assets/js/color-picker.js', __FILE__ ), array( 'wp-color-picker' ), false, true );
	
	// LOGO - Media
	wp_enqueue_media();
	wp_enqueue_script( 'razorpay-logo-upload', plugins_url('assets/js/media-upload.js', __FILE__ ), array( 'jquery' ), false, true );

} //END-js_dependency

/**
 * DUBUG
 * Check File : If exists & if Supported
 **/
function razorpay_check_file($url){

    $razorpay_check_file_msg = array(); //Declear variable
    $razorpay_check_file_error = 0; //Declear No-Error to start with.
    $razorpay_supported_img_mime = array('image/png', 'image/jpeg', 'image/gif', 'image/x-icon', 'image/svg+xml'); // List of Supported Image Mime Types.

    // Check if URL is Valid
    if ( !filter_var($url, FILTER_VALIDATE_URL) === FALSE ) {
        // Check if file exsists 
        //EXIOSTS___if ( file_exists($url) == TRUE ) {
            $razorpay_logo_check_filetype = wp_check_filetype($url); // FIND the Mime Type
            if ( !in_array($razorpay_logo_check_filetype['type'], $razorpay_supported_img_mime) ) {
                $razorpay_check_file_msg[] = __('Supported file types are "png/jpg/gif/ico"', 'razorpay');				
                $razorpay_check_file_error=1;
            }								
        //EXIOSTS___} else {
        //EXIOSTS___    $razorpay_check_file_msg[] = __('The file is not accessable at ', 'razorpay') . '<strong>' . $url . '</strong>';				
        //EXIOSTS___	$razorpay_check_file_error=1;
        //EXIOSTS___}//END-{else}-if_exsists
    } else { //Means File does not exisit.
        $razorpay_check_file_msg[] = __('Logo URL must start with "http://" -or- "https://"', 'razorpay');				
        $razorpay_check_file_error=1;
    }//END-{else}-check_logo
    
    if ( $razorpay_check_file_error == 1 ) {
        return $razorpay_check_file_msg;
    } else {
        return array('ok');
    }

}//END-razorpay_check_file
